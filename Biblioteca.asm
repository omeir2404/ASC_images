...
bmpFileDesc         dq  0
; mensagens de erro
errMsgOpenTxt       db "Erro ao abrir o TXT", LF, NULL
...
;--------------------------------------------------------------------
; readTextFile
; Objetivo: Ler um ficheiro de texto (TXT) para a memória
; Entrada : 
;   RDI - Endereço de memória para string com o nome do ficheiro a ler
...
;--------------------------------------------------------------------
readTextFile:
    push rsi ; salvaguarda o RSI para depois
    ; 1) abre o ficheiro
     mov rax, SYS_open
    ...
    cmp rax,0
    jl errorOnOpenTxt
    mov [msgFileDesc], rax ; guarda o file descriptor na variável
    ; 2) lê o ficheiro
    mov rax, SYS_read
    mov rdi, [msgFileDesc]
    pop rsi ; recupera o endereço do buffer da mensagem, que estava salvaguardado na pilha
    mov rdx, MAX_MSG_SIZE
    syscall
    cmp rax,0
    jl errorOnRead
    push rax  ; salvaguarda o valor de rax, pois este contem a quantidade de bytes lidos da mensagem
    ; 3) fecha o ficheiro
    mov rax, SYS_close
